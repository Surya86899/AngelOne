                    ...
                    if future_date in data.index:
                        ...
                        if not targetnotach and sell_candidate['High'] >= (positions[-1][2] * 1.04):
                            if data.at[future_date, 'DEMA_5'] > data.at[future_date, 'DEMA_8'] > data.at[future_date, 'DEMA_13']:
                                sl = positions[-1][2] * 1.02  # Update stop-loss to 1% down from the 4% target
                                targetnotach = True
                            else:
                        ...
                        # Continue running the trade until DEMA_5 < DEMA_8 < DEMA_13
...
df_positions.to_csv('backtest_results_with_charges.csv', index=False, mode='w')

# Append final metrics to the CSV file
with open('backtest_results_with_charges.csv', 'a', newline='') as file:
