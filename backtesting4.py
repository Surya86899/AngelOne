...
def backtest_trading_strategy(data_list, initial_capital, max_holding_period):
    ...
    buy_date = None
    start = '2024-08-22T00:00:00+05:30'
    end = '2024-12-01T00:00:00+05:30'
            ...
            if not invested and \
               ...
               today['Close'] > today['Open']:
                ...
                if max_shares > 0:
                    ...
                    sl = initial_sl
                    positions.append(('Buy', today.name.strftime('%Y-%m-%d'), today['Close'], max_shares, capital,today['Close']*1.04,sl, company_index))
                    capital -= buy_amount
                    invested = True
                    ...
                    if future_date in data.index:
                        ...
                        if not targetnotach and sell_candidate['High'] >= (positions[-1][2] * 1.04):
                            if data.at[future_date, 'DEMA_5'] > data.at[future_date, 'DEMA_8'] > data.at[future_date, 'DEMA_13']:
                                ...
                                positions.append(('Sell', sell_candidate.name.strftime('%Y-%m-%d'), sell_price, positions[-1][3], capital,today['Close']*1.04,sl, company_index))
                                invested = False
                                invested_company = None
                                sell_occurred = True
                                break
                        ...
                        # Check updated stop-loss after 4% target is achieved
                            ...
                            profit += (net_sell_amount - positions[-1][2] * positions[-1][3])
                            capital += net_sell_amount
                            positions.append(('Sell (Updated Stop Loss)', sell_candidate.name.strftime('%Y-%m-%d'), sell_price, positions[-1][3], capital, today['Close']*1.04,sl,company_index))
                            invested = False
                            invested_company = None
                            sell_occurred = True
                            break
                        ...
                        # Continue running the trade until DEMA_5 < DEMA_8
                            ...
                            profit += (net_sell_amount - positions[-1][2] * positions[-1][3])
                            capital += net_sell_amount
                            positions.append(('Sell (DEMA Condition)', sell_candidate.name.strftime('%Y-%m-%d'), sell_price, positions[-1][3], capital,today['Close']*1.04,sl, company_index))
                            invested = False
                            invested_company = None
                            sell_occurred = True
                            break
                        ...
                        # Check max holding period
                            ...
                            profit += (net_sell_amount - positions[-1][2] * positions[-1][3])
                            capital += net_sell_amount
                            positions.append(('Sell (Max Holding Period)', sell_candidate.name.strftime('%Y-%m-%d'), sell_price, positions[-1][3], capital,today['Close']*1.04,sl, company_index))
                            invested = False
                            invested_company = None
                            sell_occurred = True
                            break
    ...
    for action, date, price, quantity, cap, target, stoploss ,company_index in positions:
        if 'Sell' in action:
            trades += 1
            trade_profit = (price * quantity) - (positions[positions.index((action, date, price, quantity, cap, target, stoploss ,company_index)) - 1][2] * quantity)
            if trade_profit > 0:
                wins += 1
            else:
                losses += 1
...
df_positions = pd.DataFrame(positions, columns=['Action', 'Date', 'Price', 'Quantity', 'Capital','Target','Stop Loss','Company Index'])
